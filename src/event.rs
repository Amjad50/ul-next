...
impl KeyEvent {
    ...
    /// * `modifiers` - The modifiers that were pressed with the key.
    // TODO: explain key codes better
    /// * `virtual_key_code` - The virtual key-code associated with this keyboard event.
    /// This is either directly from the event (ie, WPARAM on Windows) or via a
    /// mapping function. You can see a full list of the possible
    /// virtual key-codes in KeyCodes.h
    ...
    // TODO: maybe have a helper function to convert virtual-key-code to text.
    /// * `text` - The actual text generated by this keyboard event.
    /// This is usually only a single character.
    /// * `unmodified_text` - The text generated by this keyboard event before
    ...
    /// * `is_keypad` - Whether or not this is a keypad event.
    /// * `is_auto_repeat` - Whether or not this was generated as the result
    /// of an auto-repeat (eg, holding down a key)
    /// * `is_system_key` - Whether or not the pressed key is a "system key".
